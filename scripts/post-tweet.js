...
async function run() {
  ...
  try {
    ...
    // Intentamos limpiar popups que pudieran bloquear la interacción
    ...
    if (isLoginPage) {
      ...
      if (isHeadless) {
        console.error('❌ Error fatal: La sesión no está autenticada y se está ejecutando en modo Headless (nube).');
        console.error('Verifica que el secreto TWITTER_AUTH_TOKEN en GitHub esté vigente y no haya sido revocado por X.\n');
        await context.close();
        process.exit(1);
      }

      console.log('Por favor, inicia sesión manualmente en la ventana del navegador.');
      console.log('Una vez que estés en el inicio (Home) de tu cuenta de Twitter, presiona ENTER aquí en la consola para continuar...\n');
      ...
      console.log('✅ ¡Sesión persistente detectada correctamente en X!');
    }

    console.log('✍️ Localizando área de redacción del tweet (Lexical/Draft.js contenteditable)...');
    ...
    console.log('✍️ Enfocando y tipeando mensaje vía eventos de teclado...');
    ...
    // Escribimos el tweet usando keyboard.insertText para activar el estado interno de React/Lexical
    await page.keyboard.insertText(tweetText);
    await page.waitForTimeout(500);

    // Forzamos un microevento para asegurar que el botón se reactive
    ...
    // Escuchamos la llamada GraphQL CreateTweet para verificar que Twitter realmente lo acepte
    const tweetResponsePromise = page.waitForResponse(
      ...
      console.warn('⚠️ No se capturó la respuesta GraphQL de CreateTweet dentro del timeout:', err.message);
      return null;
    });
    ...
    if (apiResponse) {
      ...
      console.log('⏳ Esperando confirmación visual de la interfaz...');
      await page.waitForTimeout(5000);
    }

    // Comprobamos si apareció un mensaje de éxito o si el box se vació
    ...
    console.error('❌ Ocurrió un error crítico durante la automatización de Twitter:', error.message || error);
    if (isHeadless) {
      // En modo headless en CI/CD, propagamos el error para no enmascarar fallos
